...
def _build_client() -> TestClient:
    ...
    with engine.begin() as conn:
        ...
        for article in [
            {'id': 1, 'source': 'elpais', 'title': 'Gobierno y energia', 'url': 'https://example.com/a1', 'published_at': created, 'scraped_at': created, 'section': 'politica', 'author': 'Reporter 1', 'summary': 'Resumen del gobierno', 'article_text': 'Texto articulo uno con contexto suficiente.', 'tags': 'energia,politica'},
            {'id': 2, 'source': 'elmundo', 'title': 'Mercados y energia', 'url': 'https://example.com/a2', 'published_at': created, 'scraped_at': created, 'section': 'economia', 'author': 'Reporter 2', 'summary': 'Resumen del mercado', 'article_text': 'Texto articulo dos con contexto suficiente.', 'tags': 'energia,economia'},
        ]:
            conn.execute(text("""
...
def test_explorer_points_supports_cluster_and_outlier_filters() -> None:
    ...
    outlier_response = client.get('/api/v1/semantic/explorer/points', params={'outlier_only': 'true'})
...
def test_explorer_article_detail_returns_analysis_fields(monkeypatch) -> None:
    client = _build_client()
    monkeypatch.setattr('src.semantic.dbstore.nearest_neighbors', lambda _session, article_id, limit: [])
