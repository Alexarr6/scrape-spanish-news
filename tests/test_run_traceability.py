...
class RunTraceabilityTests(unittest.TestCase):
    def test_manifest_points_to_canonical_archived_run(self):
        ...
        archived_run_root = repo_root / "runs" / "20260314-1212-8ff9"
        manifest_path = archived_run_root / "run_manifest.json"
        self.assertTrue(manifest_path.exists(), "archived run_manifest.json is required")
        ...
        self.assertEqual(canonical_run_root.parent.name, archived_run_root.parent.name)
        self.assertEqual(canonical_run_root.parts[-2:], archived_run_root.parts[-2:])
