...
class MatchingCorpusPipeline:
    ...
    def _select_articles(
        ...
        *,
        ...
        for row in rows:
            local_date = _local_published_date(row.published_at)
        ...
        *,
        ...
        for article in rows:
            ...
            selection.local_published_date = (
                chosen["local_date"] if chosen else _local_published_date(article.published_at)
            )
...
def _text_blob(row: ArticleORM) -> str:
    return " ".join(
        [
...
def _detect_bucket(text: str) -> str:
    best_bucket = ""
    best_score = 0
...
def _local_published_date(value: datetime | None) -> date | None:
    if value is None:
        return None
